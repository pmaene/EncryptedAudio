...
We decided on 128-bit keys because this is the smallest length defined by the NIST standard which is still higher than our required security level of 80 bits. {\color{red}@TODO:We zeggen in de vorige paragraaf dat we toch wel 80 bits gebruiken?}
...
\begin{itemize}
    ...
    \item (3) For the HMAC, we will calculate the keyed hash of the entire packet (except for the HMAC field itself, of course). This means that the AES encrypted packet will be used as an iput for the hash function.{\color{red}@TODO:is dit juist?} We will use a keyed SHA-3 hash here as well, truncated to 20 bytes which is the minimum required length for 80-bit security.
\end{itemize}
