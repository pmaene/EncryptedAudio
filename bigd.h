...
/* New print functions in [v2.3] */
void bdPrintHex(const char *prefix, BIGD b, const char *suffix);
	/* Print b in hex format with optional prefix and suffix */

void bdPrintDecimal(const char *prefix, BIGD b, const char *suffix);
	/* Print b in decimal format with optional prefix and suffix */
...
void bdPrint(BIGD b, size_t flags);
	/* Print b to stdout (see flags above) */
...
size_t bdConvToHex(BIGD b, char *s, size_t smax);
	/* Convert big digit b into string s of hex characters
	   ...
	   If s is NULL or smax == 0 then just return the required number. */
